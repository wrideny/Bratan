...
try:
    ...
    raise ValueError(f"ADMIN_ID must be a numeric Telegram user ID, got: {os.getenv('ADMIN_ID')!r}")
...
def menu():
    ...
    kb.add(InlineKeyboardButton("➕ Добавить звук", callback_data="add"))
...
async def add(cb: types.CallbackQuery):
    user_state[cb.from_user.id] = "wait_voice"
    await cb.message.answer("Отправь голосовое 🎤")


@dp.message_handler(content_types=["voice"])
async def voice(msg: types.Message):
    if user_state.get(msg.from_user.id) != "wait_voice":
        return
    user_state[msg.from_user.id] = "wait_name"
    user_state["file_id"] = msg.voice.file_id
    await msg.answer("Теперь отправь название")


@dp.message_handler()
async def text(msg: types.Message):
    if user_state.get(msg.from_user.id) != "wait_name":
        return
    name = msg.text
    file_id = user_state.get("file_id")
    cursor.execute("INSERT OR REPLACE INTO sounds VALUES (?, ?, ?, 0)", (name, file_id, "default"))
    conn.commit()
...
async def show_favorites(cb: types.CallbackQuery):
    cursor.execute("SELECT name FROM sounds WHERE favorite=1")
    sounds = cursor.fetchall()
    if not sounds:
    ...
    for s in sounds:
        kb.add(InlineKeyboardButton(s[0], callback_data=f"play:{s[0]}"))
    await cb.message.answer("⭐ Избранное:", reply_markup=kb)
...
async def inline(q: types.InlineQuery):
    ...
    for name, file_id in sounds:
        if q.query.lower() in name.lower():
            results.append(InlineQueryResultVoice(id=name, voice_url=file_id, title=name))
